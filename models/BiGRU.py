...
from tensorflow.keras.layers import GRU, Dense, Bidirectional, Masking
import pickle
import warnings
import argparse
import os
...
class BiGRU(object):
    ...
    def create_model(self, in_dim, out_dim, num_hidden, r_dropout, padding_value):
        ...
        model_BiGRU.add(Masking(mask_value=padding_value, input_shape=in_dim))
        model_BiGRU.add(Bidirectional(GRU(num_hidden, input_shape=in_dim, return_sequences=True, recurrent_dropout=r_dropout)))
        model_BiGRU.add(Dense(out_dim))
        model_BiGRU.compile(loss='mean_squared_error', optimizer='adam')
        return model_BiGRU
    ...
    def prep_data(self, X, y, X_mask, y_mask, valid_data, **kwargs):
        ...
        X = X_ # np.concatenate((X_, m_, d_), axis=2)
